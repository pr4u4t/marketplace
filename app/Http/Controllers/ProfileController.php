...
class ProfileController extends Controller{
    ...
    public function change2fa($turn){
        try{
            ...
            session()->flash('success', 'You have changed your 2FA setting.');
        }catch (RequestException $e){
            session()->flash('errormessage', $e->getMessage());
        }
    ...
    public function changeJSWarn($turn){
        try{
            auth()->user()->setJSWarn($turn);
            session()->flash('success', 'You have changed your JS warning setting.');
        }catch (RequestException $e){
            session()->flash('errormessage', $e->getMessage());
        }
        return redirect()->back();
