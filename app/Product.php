...
class Product extends Model
{
    ...
    /**
     * Return collection for shop page
     *
     * @return mixed
     */
    public static function allProducts($by = null, $dir = null){
        $page = (request()->has('page')) ? request()->page : 1;
        $ret = config('app.cache') ? Cache::remember('products:all:'.$page,600,function() use ($by, $dir){
            return Product::where('active', 1)->orderBy($by ?? 'created_at', $dir ?? 'desc')->paginate(9);
        }) : self::where('active', 1)->orderBy($by ?? 'created_at', $dir ?? 'desc')->paginate(9);
    ...
    public function frontImage() {
        ...
        $ret = $this->images()->where('first', true)->first() ?? $this->images()->first();

        if(!is_file(storage_path('app').'/public/'.$ret->image)){
            $placeholderImage = new Image;
            $placeholderImage->image = '../img/product.png';
